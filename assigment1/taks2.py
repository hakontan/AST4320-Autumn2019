...
def Hubble(a, Om, Ol):
    """
    Returns the Hubble parameter for a chosen cosmology.
    """
...
def solver(Om, Ol, plot=False):
    """
    Solves the ordinary ODE using Euler-Cromer.
    """
    delta = np.zeros(N)
    deltadot = np.zeros(N)
    ...
    for i in range(N - 1):
        ...
        deltadot[i + 1] = deltadot[i] + da * D_deltadot(H, a[i], delta[i], delta[i + 1])
        delta[i + 1] = delta[i] + da * D_delta(deltadot[i + 1], a[i] * H)
        
    if plot == True:
        plt.loglog(
            a, delta, label=r"$\Omega _m$ = {0}, $\Omega _m$ = {1}".format(Om, Ol)
        )
        plt.xlabel("log(a)")
...
def growth_factor(Om, Ol):
    """
    Plots the growth factor.
    """
    z = 1 / a - 1
    delta, deltadot = solver(Om, Ol)
    f = np.power(Hubble(a, Om, Ol), -1) * (deltadot / delta)
    plt.loglog(z, f, label=r"$\Omega _m$ = {0}, $\Omega _m$ = {1}".format(Om, Ol))
    plt.xlabel("log(z)")
